...
# 🔴 Les quatre vues du gabarit Le Wagon (lewagon/projects/117), dans
# l'ordre où elles s'affichent. La PREMIÈRE est celle sur laquelle on
# atterrit, donc c'est le Kanban et pas un tableau.
VIEWS = [
    ("Kanban", "BOARD_LAYOUT"),
    ("Prioritized backlog", "BOARD_LAYOUT"),
...
def ensure_views(project_id):
    """Create the four views, and drop GitHub's default table.

    🔴 This function exists because the line it replaces was WRONG. The
    script used to end by telling the user to add a board view by hand,
    saying "creating a view is not exposed by the API". It is:
    `createProjectV2View` has been a public GraphQL mutation for a while.
    Checked by introspection on 25/08/2026, after a project generated by
    this very script sat table-only for a day and read as a broken board.

    ⚠️ What genuinely is NOT exposed is the GROUPING.
    `ProjectV2ViewConfigurationInput` carries `visibleFieldIds` and
    nothing else — no `groupBy`. So the columns are still one gesture in
    the UI, and the closing message now asks for exactly that one gesture
    instead of the whole view.
    """
    existing = view_names(project_id)
    if any(name in existing for name, _ in VIEWS):
        return
    ...
    for name, layout in VIEWS:
        ...
        mutation {
          createProjectV2View(input: {
            projectId: "%s" name: "%s" layout: %s
          }) { projectV2View { name } }
        }""" % (project_id, name, layout)
        result = gh(["api", "graphql", "-f", f"query={query}"], check=False)
        if result.returncode != 0:
            print(f"  ⚠️  view {name} not created: {result.stderr.strip()}")
            return

    print(f"  views: {' · '.join(name for name, _ in VIEWS)}")
    drop_default_view(project_id)


def view_names(project_id):
    query = """
    { node(id: "%s") { ... on ProjectV2 {
        views(first: 20) { nodes { id name } } } } }""" % project_id
    result = gh(["api", "graphql", "-f", f"query={query}",
                 "--jq", ".data.node.views.nodes[].name"], check=False)
    return result.stdout.split() if result.returncode == 0 else []


def drop_default_view(project_id):
    """Remove the untitled table GitHub creates with every project.

    It is called "View 1", it is first, and leaving it means landing on a
    table when the first view should be the Kanban.
    """
    query = """
    { node(id: "%s") { ... on ProjectV2 {
        views(first: 1) { nodes { id name } } } } }""" % project_id
    result = gh(["api", "graphql", "-f", f"query={query}",
                 "--jq", ".data.node.views.nodes[0].id + \" \" + "
                         ".data.node.views.nodes[0].name"], check=False)
    if result.returncode != 0:
        return

    view_id, _, name = result.stdout.strip().partition(" ")
    # Only the generated one. A view somebody named is a view somebody
    # wanted.
    if not name.startswith("View "):
        return

    # ⚠️ `deleteProjectV2View` returns a payload with NO `projectV2`
    # field — asking for one is a GraphQL error, not a permissions
    # problem. `clientMutationId` is what it will give you.
    gh(["api", "graphql", "-f", "query=" + """
        mutation { deleteProjectV2View(input: {viewId: "%s"})
                   { clientMutationId } }""" % view_id], check=False)
...
def main():
    ...
    print("One gesture left, and only one: open Kanban → ⋯ → Group by → "
          "Status, then Prioritized backlog → Group by → MoSCoW Priority.")
    print("Grouping is the one thing the API does not expose — "
          "ProjectV2ViewConfigurationInput carries visibleFieldIds and "
          "nothing else.")
